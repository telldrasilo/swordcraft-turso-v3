...
**Нецели итерации 1 (сознательно позже):** обжиг керамики, дубление кожи, отдельные процессы заказов NPC, начисление экспедиционного лута — по мере появления в реестре. Для них пока нет значения в `MaterialProcessKind`; вклад материала не запрашивается.
