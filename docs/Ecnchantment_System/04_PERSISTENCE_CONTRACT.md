...
Система зачарований зависит от цепочки хранения:

1. **Zustand persist** (`localStorage`, `swordcraft-store-v2`) — всегда; основной офлайн-источник после перезагрузки страницы.
2. **Хук `use-cloud-save.ts`** — всегда участвует как минимум как **локальный бэкап** (`swordcraft-offline-backup`) и сборщик payload; при включённом облаке дополнительно вызывает `/api/save`.
3. **Сервер Turso** (`/api/save`, `src/lib/db.ts`) — **только** если в окружении задано **`NEXT_PUBLIC_CLOUD_SAVE_ENABLED=true`**. Флаг и чеклист расширения схемы: `src/lib/cloud-save-feature.ts`. Без флага маршрут `/api/save` не используется клиентом (прямой вызов вернёт `503` + `cloudSaveDisabled`).

Если менять shape сохраняемых данных, нельзя обновлять только один звено цепочки — иначе рассинхрон persist, бэкапа и (при включённом флаге) БД.

Ниже разделы «Cloud save» и «API» описывают **полный** пайплайн — это обязательно при работе с облаком или при подготовке к его включению; для чисто локальной разработки достаточно persist + полей в `collectSaveData`/`applyLoadedData`, чтобы при будущем включении флага данные уже совпадали.
...
## Cloud save: сбор данных (и бэкап)

Источник: `src/hooks/use-cloud-save.ts` (сеть — только при `NEXT_PUBLIC_CLOUD_SAVE_ENABLED=true`).
...
Источник: `src/app/api/save/route.ts` (актуален при включённом облаке, см. `cloud-save-feature.ts`).
...
- `src/lib/cloud-save-feature.ts` (политика включения облака и чеклист)
- `src/hooks/use-cloud-save.ts`
- `src/app/api/save/route.ts`
- `src/lib/db.ts`

Если хотя бы один файл из цепочки persist → hook → (при облаке) API/БД не обновлён, контракт персистентности считается нарушенным.
