...
import type { Rarity, LocationResource } from '../../types';
...
export interface EventEffect {
  ...
  /** Шанс срабатывания эффекта (0–100), если используется в данных */
  chance?: number;
}
...
export interface EventTemplate {
  ...
  // === ЭФФЕКТЫ ===
  // Для type !== 'choice' обязательны (см. validateEventData); для choice — только choices[].
  effects?: EventEffect[];
...
/** Результат резолва: grant_material подставляется из локации при grant_location_material */
export type ResolvedEffectType = EventEffectType | 'grant_material';
...
function effectiveLootSelectionWeight(resource: LocationResource): number {
  const tier =
    resource.lootTier ??
    (PRIMARY_LOOT_MATERIAL_IDS.has(resource.materialId) ? 'primary' : 'secondary')
  const mult = tier === 'primary' ? PRIMARY_LOOT_WEIGHT_MULT : SECONDARY_LOOT_WEIGHT_MULT
  return resource.baseWeight * mult
}
